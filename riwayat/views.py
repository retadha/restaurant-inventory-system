...
from propensi.utils import is_manager, is_gudang_pusat, is_restoran
...
def pembelian(request):
    ...
    pembelian = Request.objects.filter(
        id_gedung__status__exact="0", status="3")
    context = {
...
def pembelian_lines(request):
    ...
    lines = Inventory_Line.objects.filter(
        id_request__id_gedung__status__exact="0", id_request__status__exact="3")
    context = {
        'title': "Riwayat Pembelian Lines",
        'lines': lines
    }
...
def list_riwayat_penjualan(request):
    if not (is_manager(request) and is_restoran(request)):
        return render(request, 'error/403.html')
    ...
    response = {
        ...
        'riwayat_penjualan': riwayat_penjualan
        }
